  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    let update = propsDiff
      ? Object.keys(prevProps)
          ...
          .map(
            (k) =>
              `"${k}": {"prevProps":${JSON.stringify(
                prevProps[k]
              )}, "newProps":${JSON.stringify(this.props[k])}}`
          )
          .toString()
      : "";
    update += stateDiff
      ? Object.keys(prevState)
          ...
          .map(
            (k) =>
              `"${k}": {"prevState":${JSON.stringify(
                prevState[k]
              )}, "newProps":${JSON.stringify(this.state[k])}}`
          )
          .toString()
      : "";
    if (update !== "") {
  ...
  render() {
    ...
    return (
      <div>
        <p>
          {this.state.isJavascript ? (
            <span>
              ...
              or other HTML tags.
              <br />
              As a good practice, run the js trough a validator such as{" "}
              <a href="https://codebeautify.org/jsvalidate">
                code beautify
              </a>{" "}
              before saving.
            </span>
        ...
        <p>
          Note: you need to reload the page to observe your newly saved{" "}
          {this.state.isJavascript ? "javascript" : "css"}.
        </p>
      </div>
    );
  }
...
class ManageAdditionalTabs extends React.Component<
  CssJsTabsProps,
  CssJsTabsState
> {
  /**
    ...
    return (
      <div>
        <div className="grid-2-cols">
          <label htmlFor="typeCode" className="label-for-input">
            Type of code:&nbsp;
          </label>
          <div>
            CSS{" "}
          ...
          <label htmlFor="tabNameToManage" className="label-for-input">
            Tab name:&nbsp;
          </label>
          <input
            id="tabNameToManage"
          ...
          />
          <label htmlFor="enabled" className="label-for-input">
            Enabled:&nbsp;
          </label>
          <div>
            {" "}
            {/* Enables the checkbox to be aligned to the left*/}
            <input id="enabled" type="checkbox" style={{ marginLeft: 0 }} />
          </div>
        </div>
        <button
          className="custom-css-js-button"
...
class ModalInterface extends React.Component<
  CssJsModalInterfaceProps,
  CssJsTabsState
> {
  constructor(props) {
    super(props);
      ...
      () => {
        if (this.state.tabNameToManage) {
          this.setState({ enableCrudButtons: true }, () => {
            console.log(
              `ModalInterface.onTextSettingsChange/]TN\n\n`
            ); /*\n/state: {
                "css": "${this.state.css}"
                ,"enableCrudButtons": ${this.state.enableCrudButtons}
                ,"javascript": "${this.state.javascript}"
  ...
  modifyTabsVarBasedOnUpsert(state, enabled, isJavascript) {
    ...
    const tabToUpsert = {
      ...
      tabStyle: { color: `var(--dark-${isJavascript ? "yellow" : "blue"})` },
      testId: state.tabNameToManage + "Tab",
    };
    ...
    if (!tabExisted.length) {
      ...
      Array.from(domTabs).map((k) =>
        k.setAttribute("aria-setsize", (domTabs.length + 1).toString())
      );
      return tabs;
    } else {
  ...
  deleteTab(state) {
    ...
    const doDelete = window.confirm(
      `Are you sure you want to delete the code from the ${state.tabNameToManage} tab?`
    );
    if (!doDelete) {
    ...
    const tabsAfterDeletion = tabs.filter(
      (tab) => tab.label !== state.tabNameToManage
    );
    ...
    */
    ...
    Array.from(tabsDom).map((k) =>
      k.setAttribute("aria-setsize", (tabs.length - 1).toString())
    );
    //tabInDom.remove();
    console.log(
    ...
    return (
      <React.Fragment>
        <Tabs
          ...
          onChange={(index, _) =>
            console.log(
              `AuiTabs.render/selectedTab: ${this.state.tabs[index].label}.`
            )
          }
        >
          <TabList>
          ...
          {this.state.tabs.map((val, i) => (
            <TabPanel
              testId={val.testId + "TabPanel"}
              key={val.testId + "TabPanel"}
            >
              <this.Panel
                testId={val.testId + "TPanel"}
                key={val.testId + "TPanel"}
              >
                {val["isJavascript"] !== undefined ? (
                  ...
                  <ManageAdditionalTabs
                    ...
                    Panel={this.Panel}
                    deleteTab={this.deleteTab.bind(this)}
                    modifyTabsVarBasedOnUpsert={this.modifyTabsVarBasedOnUpsert.bind(
                      this
                    )}
                    onTextSettingsChange={this.onTextSettingsChange.bind(this)}
                    renderCont={this.renderCont.bind(this)}
